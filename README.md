...
標準WIA/TWAIN経由では内蔵LEDを消灯したまま取得できなかったため、2026-08-15にfi-65F実機を分解し、光源を物理・電気の両面から切り分けました。この節は成功手順ではなく、**実機で確認できた事実、推定、未完了の試行を区別して残すための記録**です。

### 15.1 scanner cameraとしての前提確認

ハード改造に入る前の光学実験では、fi-65Fを暗箱内に置き、外部レンズで原稿面付近へ結像した像をスキャンして、外部光学系による像が画像として取得できることを確認しています。蛍光灯を被写体にした試験では像を認識できた一方、ラインスキャンと照明フリッカーの組み合わせと考えられる縞も確認しました。

このためscanner camera化の主なハード課題は、CIS自体を変更することより、**スキャン中に内蔵RGB LEDの照明を撮像面へ入れないこと**です。

### 15.2 光源ユニットと配線で確認したこと

分解したCIS/光源ユニットでは、長尺の白色導光部と端部のLED光源が近接して配置されています。LED側の基板には次の4端子表記があります。
...
TWAIN `ICAP_LIGHTSOURCE`でRED/BLUEを指定した際に実際の発光色が変化したことから、少なくともR/Bを含む光源制御が実LEDへ到達することはソフトウェア試験でも確認済みです。

電源OFFで配線を追った実測では、**C端子がCIS基板側15-pinコネクタのpin 2へ接続されていることを確認**しました。ただし、Cがcommon anode / common cathode / その他の共通制御線のどれであるかは確定していません。

### 15.3 光源を外した場合のエラー

光源ユニットを外してスキャンしたところ、PaperStream IPから次の異常が報告されました。
...
次に、LED/光源を**電気的には接続したまま**、本来の位置から外してCISへ光が入らない状態でスキャンしました。この場合も同じ光量異常になりました。

したがって、少なくとも次は実機で確認済みです。

- LED負荷を電気的に接続しているだけでは正常初期化できない
- CIS側が実際に一定量の光を受けることが必要
- 光量異常は単なるLED断線検出だけでは説明できない

また、動作開始時にはRGB LEDがフラッシュのような強い閃光を出す挙動を確認しました。これは白基準・光量校正に使われている可能性が高いと考えていますが、**校正処理であること自体は推定であり、fi-65Fの内部仕様として確認したものではありません**。

### 15.4 機械式遮光を見送った理由

初期化時だけ光を通し、その後にLEDと導光板の間を遮る機械式シャッターも検討しました。しかし実機ではLED出射部と導光板端面がほぼ密着しており、可動遮光板を入れるためのクリアランスは1 mm未満です。

このため、既存光学系を保持したままLED/導光板間へシャッターを追加する方法は、工作難度と信頼性の点から現時点では採用していません。

### 15.5 電気的に初期化後だけLEDを切る試行

次の方針として、初期化に必要な発光だけを残し、初期化後にLED共通線を電気的に切る方法を検討しました。
...
C端子と15-pinコネクタpin 2の導通を確認したため、main board側の細ピッチコネクタ近傍からpin 2ネットを引き出すジャンパーを追加しました。

しかし、この作業中にコネクタ近傍のランドを損傷しました。組み戻した後は、本来待機中は消灯するLEDが**赤色で連続点灯する状態**になっています。現時点ではpin 2への接続位置・ランド損傷・隣接ネットへの影響のいずれかが原因と推定していますが、故障点は確定していません。

そのため、この個体については改造前の正常状態を維持しているとは扱わず、以後のLED挙動をfi-65F本来の仕様判断には使用しません。
...
| 光源ユニットを外して正常スキャン | 不可 | PaperStream IPが光量異常を報告 |
| LEDを接続したままCISへ光を入れずに正常スキャン | 不可 | 同じ光量異常を報告 |
| 初期化時に強いLED発光がある | 確認済み | 実機観察 |
| 強い発光が白基準校正である | 推定 | 挙動からの推定。内部仕様未確認 |
| LED基板の`C`が15-pin pin 2へ接続 | 確認済み | 電源OFFでの導通確認 |
| pin 2がLED専用の共通電源/共通returnである | 未確定 | 極性・回路機能を未解析 |
| 初期化後にpin 2を切れば走査を継続できる | 未検証 | ジャンパー作業中に基板損傷 |
| 赤LED連続点灯の原因 | 未確定 | pin 2周辺の作業影響を疑うが未解析 |

### 15.7 再開する場合の方針

この実験を再開する場合は、まず正常状態の個体または修復済み個体でベースラインを取り直します。細ピッチコネクタのランドへ直接ジャンパーを追加する方法は再現性が低いため、次のような**可逆なinterposer / breakout**を優先します。

- 15-pin FFC/FPC延長・中継基板で1線だけ外へ引き出す
- FFCの対象導体だけを絶縁し、別配線でバイパスする
- 同一ネット上の十分な面積を持つtest point / viaが確認できればそこから引き出す

そのうえで、手動スイッチ等を使い、まず「初期化時はLED ON、強い初期化発光後にOFF」という最小試験を行います。これが成立した場合に限り、MOSFET/load switch等による自動化を検討します。

現時点では、**LED無発光での本スキャンは未達**です。一方で、ソフトウェア側の制御限界、光量チェックの存在、LED/導光系の物理構造、C端子とpin 2の接続まで切り分けられたため、次に試すべき介入点は以前より限定できています。
